...
<script>
	...
	export default {
		...
		methods: {
			//请求数据
			async loadData(){
				this.cartList = await this.$http('api/cart/list',{userId:1});
			},
			//监听image加载完成
			...
			async deleteCartItem(id){
				console.log('删除' + id)
				await this.$http('api/cart/del',{id});
			...
			async clearCart(){
				...
				await this.$http('api/cart/remove',{userId});
				window.location.reload()
			},
			//计算总价
			...
			async createOrder(){
				console.log('开始创建订单')
				let id = await this.$http('api/order/add',{
